...
**P0-9. Characterisation tests for the guest action paths** (blocks P0-4, part of G6) *(IN PROGRESS 2026-08-27: harness plus **20 of 41 rows** green with committed snapshots (Confirm 7, Cancel 13). `src/app/api/confirm/characterisation/harness.ts` fixes the frozen shape `{status, body, dbWrites[], externalCalls[]}` with `vi.setSystemTime`, fixture ids and a module-then-fn sort, and uses an `after()` stub that RUNS its callback (the precedent suite's bare `vi.fn()` swallows every deferred comm). **21 rows remain**: Modify 19, cross-cutting 2. The Confirm slice corrected one matrix row (see the HMAC note below), and the Cancel slice's 502 row was mutation-tested: disabling the route's refund guard fails that row and only that row.)*
