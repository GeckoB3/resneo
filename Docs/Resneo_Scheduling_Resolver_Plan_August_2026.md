...
3. **One weekly-hours editor replaces three** — specified as decision (K) below — and **one date-override editor replaces the venue closure editor and the leave panel**.
  ...
  **Step 6 is partly already there:** `/dashboard/calendar-availability` carries an explainer, "How calendar hours and business hours work together", linking to Settings. What remains is showing the venue's actual hours beside the calendar's rather than describing the relationship in prose.
