...
Steps 1 to 5 are now implemented (see the implemented notes above): they covered
the biggest staff and guest pain points with no migration. **Step 6 is all that
remains** — lower-priority, front-end-only polish: two-panel consolidation (U14),
self-service resubmit / "fix a mistake" path (U15), expired-link notice (U16),
option-value collision guard (U8), library preview (U9), navigation
consolidation (U6), and bulk type actions (the unshipped half of U5).
