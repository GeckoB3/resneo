...
| `Docs/MOBILE_API.md` | The contract the React Native app already depends on | `createVenueRouteClient` for `/api/venue/*`. **Treat its route table as incomplete**: it lists 11 venue routes and no account routes, while the shipped app also calls six `/api/account` and `/api/v1` pairs (§5D.0). Its `reserveniapp://callback` line is stale; the scheme is now `resneo://` |
...
**Status at 2026-08-26: F1, F3 and F6 are done, F2 is partly done. F4, F5, F7 and F8 need a human with credentials or authority this plan does not have.**
...
| **F2 ◐** | **PARTLY DONE 2026-08-26**: `.env.e2e.example` is committed, with every variable the suite reads and a `.gitignore` negation so the template is tracked while the real `.env.e2e` stays ignored. **Still needed from you: the Actions secrets, `RUN_E2E_SMOKE=true`, and Stripe Connect on the fixture venue.** **Make the e2e job runnable.** Set `RUN_E2E_SMOKE=true` and create the Actions secrets it needs; the workflow's own comments say they do not exist. Add the `.env.e2e` template referenced by `playwright.config.ts`, `global-setup.ts` and the seed script but absent from the repo. Provision Stripe Connect on the fixture venue. | Four acceptance criteria say "green in CI". None can be demonstrated today. | | ☐ |
...
| **F8** | **Confirm `resneo://callback` is in the Supabase Redirect URLs allowlist.** `Docs/MOBILE_API.md:58` documents adding `reserveniapp://callback`, which is the pre-rebrand scheme. *(Attempted 2026-08-26: `/auth/v1/settings` does not expose the allowlist, so this is dashboard-only and cannot be checked from the repo.)* | The shipped app's magic-link and password-reset sign-in produce `resneo://callback`. If it was never added, **those flows are broken in production today**. Only the dashboard settles it. | | ☐ |

| **F9** | **Compare staging and production auth settings before Phase 3.** The staging project reports `mailer_autoconfirm: true` (Confirm email off). §5A already requires the two projects to match before Phase 3, and `claim_user_account()`'s `email_confirmed_at` gate behaves differently depending on it, so P3-4c's acceptance is only meaningful if they agree. *(Surfaced 2026-08-26 while checking F8.)* | A staging pass on P3-4 proves nothing if the auth settings differ from production. | | ☐ |
...
- **And the documented allowlist entry is for the wrong scheme.** `Docs/MOBILE_API.md:58` instructs an operator to add **`reserveniapp://callback`** to Supabase's Redirect URLs. That is the pre-rebrand scheme. Whether **`resneo://callback`** was ever added in its place is unverified and only the Supabase dashboard settles it. **This is a production question, not a Phase 3 one:** the shipped 1.0.7 magic-link and password-reset flows call `Linking.createURL('callback')`, which produces `resneo://callback`, so if it is absent those flows are broken for real users today. Check it during pre-flight.
