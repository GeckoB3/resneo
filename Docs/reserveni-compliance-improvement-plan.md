...
Steps 1, 2 and 3 are now implemented (see the implemented notes above): they
covered the biggest staff and guest pain points with no migration. Step 4
(blocked-online guidance + returning-client lookup) is the recommended next
pickup, and is the first remaining one to touch the data model or shared message
copy; Step 5 also does, and everything else is front-end-only.
