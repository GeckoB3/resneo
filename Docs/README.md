...
| `collective-one-venue-plan.md` | Not implemented. Written 2026-09-13 from a multi-agent forensic audit of venue collectives (eight subsystem maps, a cross-examination, two competing architectures, two red teams). Records how collectives work today, 42 split-brain cases and 59 bugs, and recommends host-managed replicas: the host's service is the master, members hold locked copies written by one audited engine, calendar assignments are the only record of which calendar offers what, and leaving is lossless. Companions: `collective-one-venue-ux-spec.md` (pages and copy) and `collective-one-venue-test-plan.md` (147 tests, invariants, rollout gates). Decisions D1 to D51 are the owner's; D9 asks for legal counsel before build. Supersedes `collective-service-sync-plan.md` in design. Second forensic pass 2026-09-14 at `c0b5eb0` widened it to reporting, operations, multi-venue people and plan tiers |
