...
export default async function SettingsPage({
  ...
  const { data: fullVenue, error: fullErr } = await staff.db
    .from('venues')
    .select('id, name, slug, address, phone, email, website_url, cover_photo_url, logo_url, cuisine_type, price_band, no_show_grace_minutes, kitchen_email, owner_booking_notification_enabled, owner_booking_notification_email, google_review_url, review_request_enabled, communication_templates, opening_hours, venue_opening_exceptions, booking_rules, deposit_config, availability_config, stripe_connected_account_id, timezone, table_management_enabled, combination_threshold, pricing_tier, plan_status, billing_access_source, free_access_granted_at, free_access_granted_by, free_access_reason, subscription_current_period_start, subscription_current_period_end, calendar_count, booking_model, enabled_models, active_booking_models, terminology, sms_monthly_allowance, stripe_subscription_id, created_at, require_account_login_for_bookings, in_person_payments_enabled, feature_flags, embed_accent_colour, booking_page_config')
    .eq('id', venueId)
    .single();
