 ...
 * sites. All three fetchers already derive `venueOpeningExceptions` from
...
describe('attachVenueClockToAppointmentInput exception precedence (SA-L1)', () => {
  it('keeps a fetcher-derived list instead of replacing it with legacy JSON', () => {
    const derived = [
      { id: 'from-blocks', date_start: '2030-06-05', date_end: '2030-06-05', closed: false, periods: [{ open: '10:00', close: '14:00' }] },
    ];
    const input = baseInput({ venueOpeningExceptions: derived });

    attachVenueClockToAppointmentInput(input, { ...VENUE, venue_opening_exceptions: LEGACY_JSON });

    expect(input.venueOpeningExceptions).toEqual(derived);
  });
  ...
  it('still fills from legacy JSON when the caller genuinely has nothing', () => {
    ...
    expect(input.venueOpeningExceptions).toHaveLength(1);
    expect(input.venueOpeningExceptions?.[0]?.id).toBe('legacy-1');
  });

  it('does not let legacy JSON revive an empty derived list, which means "no closures"', () => {
    const input = baseInput({ venueOpeningExceptions: [] });

    attachVenueClockToAppointmentInput(input, { ...VENUE, venue_opening_exceptions: LEGACY_JSON });

    expect(input.venueOpeningExceptions).toEqual([]);
  });
  ...
  it('prefers explicitly passed venueBlocks over both', () => {
    ...
    expect(input.venueOpeningExceptions?.map((e) => e.id)).toEqual(['blk-1']);
  });
  ...
  it('leaves the field untouched when the venue row carries no legacy column at all', () => {
    ...
    expect(input.venueOpeningExceptions).toBeUndefined();
  });
});
