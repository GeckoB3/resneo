...
| 2026-08-31 | Bounded the staff email fallback in `resolveStaffIdentityUncached` to rows where `user_id IS NULL`, and matched the rule in `/auth/choose-destination`. Not a register finding: it was found while answering a mobile-repo question about whether the fallback was still needed. A claimed staff row could be matched by email, because `staff.email` is not kept in step when an auth email changes elsewhere, so a later signup on the stale address inherited the original owner's staff id, venue and role. Distinct from §8's "email change acquiring a staff row", which remains blocked. The fallback itself cannot be removed: `staff/invite` inserts rows with no `user_id`. Locked by `src/lib/venue-auth.staff-identity.test.ts`. |
