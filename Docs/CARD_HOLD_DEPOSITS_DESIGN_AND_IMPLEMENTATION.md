...
### D3. Charge is gated on booking status = `No-Show` (or a kept late cancellation), staff-explicit, admin-only.

- Charge enabled only when `status='No-Show'` OR (`status='Cancelled'` with `booking_card_holds.late_cancellation_at` set, the §9.3 late-cancellation amendment), plus `deposit_status='Card Held'`, hold not released, within the charge window.
- **A cancellation before the booking's `cancellation_deadline` releases the hold**; a cancellation after it (guest self-cancel or staff-recorded) KEEPS a saved hold chargeable, stamped `late_cancellation_at`. Venue-initiated cancels (cascades, crons, GDPR) always release. Nothing is ever charged automatically; charging stays a manual admin action.
...
(There is deliberately no ad-hoc "admin release" of a saved hold on a LIVE booking: `waive` applies only to `Pending` unsaved holds; a live saved hold is released only by cancel, refund, or expiry. The one exception is the `release_hold` deposit-route action, valid only for a hold KEPT by a late cancellation on a Cancelled booking (§9.3 amendment), releasing with `release_reason:'admin'`. Re-sending the card link on a released hold is 409, 9.2b.)

Booking `status` machine unchanged. Charge eligibility = `(status='No-Show' OR (status='Cancelled' AND late_cancellation_at IS NOT NULL)) AND deposit_status='Card Held' AND released_at IS NULL AND now() <= charge_window_ends_at`.
...
2. ~~**Late-cancellation fees**~~ **Delivered July 2026** (§9.3 amendment): a cancellation after the booking's `cancellation_deadline` keeps a saved hold chargeable (`late_cancellation_at`), the charge gate accepts kept late-cancelled bookings, consent text v2 quotes the cancellation notice, and the manage page warns before a late cancel. The policy input is the existing per-entity Cancellation notice (hours); no new config was added.
...
12. **§7.2 cart review.** The quote seam is implemented as specified (`/class-cart/quote` returns `card_hold_fee_pence` per line and in the summary), and the orphaned `ClassMultiSessionCart` component renders it, but the live multi-date flow in `ClassBookingFlow` goes straight to checkout without a quote-driven review step. The guest first sees the hold fee and consent at the payment step (cart total), which carries the full §7.5 consent copy. If the cart review step is revived, the quote wiring is ready.
13. **§10.2.2 receipt.** The no-show fee receipt also states the charge date, rendered in the venue's timezone (an addition to the specified copy).
14. **§11 timeline.** The five card-hold events (`card_hold_saved` / `card_hold_released` / `card_hold_charged` / `card_hold_charge_failed` / `card_hold_charge_refunded`) are surfaced in the staff booking timeline with plain-language titles and amount/reason detail lines.
15. **§8.6 webhook completion.** Webhook idempotency was hardened so an event is marked complete only after processing succeeds (`webhook_events.completed_at`), not at claim time. A worker that dies mid-processing leaves the claim reclaimable, so an async no-show fee completed only by the webhook is retried rather than dropped. Applies to all Stripe webhooks, not just card holds.
16. **§8.3 loser/orphan PI disposal.** When a charge attempt loses the persist race, or the hold is released between the guards and the persist, its PaymentIntent is refunded if it already succeeded (not just cancelled), so no orphan charge stands; a reconciliation alert is raised only when neither disposal lands. A charge that stamps onto a hold released in the same instant also raises a reconciliation alert.
17. **§15 report access.** `report_deposit_summary` is locked to the service role (`REVOKE ... FROM PUBLIC`), and `booking_card_holds.release_reason` gained a CHECK constraint matching the five allowed reasons.
18. **§9.3 late-cancellation amendment (July 2026, delivering §19 item 2).** Card holds now honour the per-entity **Cancellation notice (hours)** via the booking's stored `cancellation_deadline`. Guest self-cancels and staff-recorded cancels run `settleCardHoldsOnCancellation`: before the deadline the hold is released exactly as before; after it, a SAVED hold is kept (`booking_card_holds.late_cancellation_at`, events row `card_hold_kept_late_cancellation`) and the admin charge action works on the Cancelled booking. Staff can instead release without charging (`release_hold` action, any staff, parity with waive); the 14-day expiry cron remains the backstop. Venue-initiated cancels (class/event cascades via `cancelStaffBookingWithNotify`, crons, webhooks, GDPR) always release, so a guest is never chargeable for a cancellation the venue made. Consent text moved to snapshot version 2 quoting the notice hours ("or if you cancel less than N hours before your booking starts"). Version 1 snapshots promised charge-free cancellation any time before the start, so the settle helper keeps ONLY holds consented under version 2+: pre-existing v1 holds always release on cancel, honouring their own terms, and age out within days of the change shipping. Guest surfaces (manage page, account page, confirmation emails) quote the deadline and warn before a late cancellation.
