-- D46 revised (owner decision 2026-09-16, "option 2"): staff move a booking to a calendar at another
-- venue of the same shared-services collective, when nothing is attached to it that cannot move.
--
-- STILL DARK: refuses anything that is not on the replicas model.
--
-- collective_move_booking(booking, target calendar, date, time, target guest, actor venue,
--                         actor staff, now) -> the new booking's id
--
--   One transaction under the collective lock:
--     * the booking is checked movable: Booked or Confirmed, not part of a visit or group, no
--       deposit, card hold, payment, tip or completed form (COLLECTIVE_MOVE_ATTACHED names which);
--     * the service, its option and its add-ons are found at the target venue through the
--       offering: the host's master, or the member's live replica, mapped by replica_of_* keys
--       (COLLECTIVE_MOVE_SERVICE when the target calendar does not offer it);
--     * a copy is inserted at the target venue with every column carried over except the ones that
--       describe what already happened to the original (attendance, reminders sent, tokens), on
--       the target calendar, service and option, at the new date and time, for the target venue's
--       own guest row (the caller finds or creates it there), keeping its price snapshots;
--     * its add-ons are copied at the prices booked;
--     * the original is cancelled by staff, and both venues' events logs say where it went.
--
--   Nothing here sends a message: the caller tells the guest once, from the new booking. Slot
--   availability is the caller's check, as it is for every staff write.
...
BEGIN
  ...
  WHERE c.status = 'active' AND c.service_model = 'replicas' AND c.paused_at IS NULL
