...
**Four tasks are enabling work and close no gap of their own**, so they are absent from the table by design: **P0-0** (the test double four other tasks need), **P0-13** (schema that makes P4-3 and P5-2 possible without breaking the shipped app), **P0-16** (the `ConfirmDialog` slot P2-2 and P2-6 need) and **P0-17** (native-compatible membership checkout, which implements C9).
...
- Settle the 401 body. `/api/venue/*` uses `'Unauthorised'` (259 sites); `/api/account/*` uses `'Unauthenticated'` (31 sites, 35 including `/api/v1`). Pick one and apply it.
...
- **While the contract is open, give the three prose-matched `/api/venue/*` errors real codes**: the calendar-cap 403, the resource-clash 409 and `'cannot be switched off'`. The app matches English sentences for all three today and will break the moment anyone edits that copy.
...
| **B5** | Enforcing `426 CLIENT_TOO_OLD` | 1.0.7 sends no `X-ResNeo-Client` and has no 426 branch and no update prompt. It would show `Request failed (426)` on every screen with no way forward. **"Header absent" must be permitted permanently** |
...
**Opportunity while the contract is open.** Three `/api/venue/*` error paths are matched by English prose today and will break the moment anyone edits that copy: the calendar-cap 403 (`BookableCalendarsManager.tsx:76-82`), the resource-clash 409 (`ResourceEditorSheet.tsx:151-159`) and `'cannot be switched off'` (`booking-settings.tsx:392`). Adding `CALENDAR_LIMIT_REACHED`, `CALENDAR_CONFLICT` and `FLAG_LOCKED` to P0-11's union, and promoting `COMPLIANCE_REQUIREMENT_UNMET` into `code` alongside its existing `error` value, lets the next app build drop all four prose matches at no coordination cost.
...
| **R2** | Code only: the P0-6 loader switch, P0-3, P0-2, P0-12, P0-16, P0-15, **P0-0**, P0-9, **P0-1a, P0-1b, P0-1d**, the `portal_events` emitters (fail-soft) and prune cron, the `user_devices` audience writer, and **tolerant `notification_preferences` readers and writers** | Depends on R1 being on production. The tolerant readers must be live before R3 |
...
**On the estimate increase, from 13 to 14.5 weeks to 18 to 20.** Phase 0 grew again, and for the same reason it grew before: this review found more things the previous version assumed were in acceptable shape. Six are new tasks rather than re-scoping:
