...
**P0-9. Characterisation tests for the guest action paths** (blocks P0-4, part of G6) *(STARTED 2026-08-27: harness plus the 7 Confirm rows green, snapshots committed. `src/app/api/confirm/characterisation/harness.ts` fixes the frozen shape `{status, body, dbWrites[], externalCalls[]}` with `vi.setSystemTime`, fixture ids and a module-then-fn sort, and uses an `after()` stub that RUNS its callback (the precedent suite's bare `vi.fn()` swallows every deferred comm). **34 rows remain**: Cancel 13, Modify 19, cross-cutting 2. The first slice already corrected one matrix row, see the HMAC note below.)*
