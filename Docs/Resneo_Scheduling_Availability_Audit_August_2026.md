...
**Round 4 shipped: `995e8803`, staging, hand-tested. Code only.** Closure-aware reminder suppression, the last item carved out of round 1. **Nothing from Phase 0 is now outstanding**, and `SA-M2`'s reminder limb is closed alongside its diary limb.
...
### PHASE 1 IS IN SCOPING, NOT STARTED. Do not schedule from §13's Phase 1 as written.

Reconnaissance to 2026-08-16 has already falsified enough of it that starting from the text would build the wrong thing:

1. **It is three programmes bundled, not one.** Only `SA-H2` and `SA-M1` are resolver work. `SA-C3` is the *fetcher* plus a new booking-UI state. `SA-M8` (booking window on the day API) and `SA-M10` (month path double-applying `custom_duration`) are unrelated to closures and were mis-filed here. `SA-M12`, `SA-M19`, `SA-L1`, `SA-L3` are small separable fixes. The bundling is most of why it is sized **L**.
2. **"Create `src/lib/availability/resolver/`" is the wrong instruction.** `venue-wide-business-hours.ts` already is that resolver and four of the five engines already use it. Building a sixth repeats the defect this document exists to describe.
3. **It picks the wrong canonical semantic.** It assumes the intersecting resolver is right and the appointment engine wrong. The shipped UI promises owners that Amended Hours let them "close early **or open late**" and "enter the hours you are actually working" — **open late only works under replace**. Unifying on intersect would break a documented promise. The operator has confirmed replace is the intended meaning.
4. **`venue-wide-business-hours.ts` is not "well tested".** It has **2** tests. And no parity test exists between any two layers, so §11.6's "single highest-leverage test investment" is entirely absent.
5. **The four resolver-using engines disagree with each other**, which this document does not record. On a weekly-closed day, classes bail out before checking, events carve out via `isWeeklyScheduleClosedForDate`, and resources do neither and block. Same venue, same Sunday, three answers.
6. **The calendar layer is unmapped here and is larger than the venue layer.** `unified_calendars` carries `working_hours`, `days_off`, `availability_exceptions`, and **two competing break models** (`break_times` and `break_times_by_day`), alongside `practitioner_leave_periods`, `calendar_blocks`, `practitioner_calendar_blocks` and per-service `custom_working_hours`. §2's matrix treats calendar hours as one row.

**Operator decisions already taken:** amended hours **replace** the weekly baseline; part-day closures **subtract** rather than widen to whole-day; part-day closures are **retained** rather than removed; scheduled instances (classes, events) are gated by explicit closures only and never by the weekly baseline; **no per-venue flag** — the parity matrix replaces it as the safety net, and each caller migrates in its own commit.

The plan supersedes §13's Phase 1 and §11.1 when it lands. Until then this section is the current state.
...
> **REGRESSION, found in production use and fixed 2026-08-16 · `51d00fd6`. The staging verification above was incomplete and said so without knowing it.**
>
> The gate above was unconditional because of a premise written into its own comment: *"Both this route and `create-group` are anonymous public flows."* **False.** Neither route has staff authentication, but both are also where the **staff mobile app** creates every multi-service visit and every group booking, with a `source` of `phone` or `walk-in`. A walk-in is by definition today, so on any venue with `allow_same_day_booking: false` staff at the counter got a `400` on a booking they were standing there taking.
>
> The correct helper, `isStaffWalkInBookingDateAllowed`, sat uncalled in the same file, and **both routes already branched on `source` several times** — for compliance context, entitlement blocking and address requirements — one of them under a comment reading "staff sources (phone/walk-in) block only on block_all". The distinction was there to be read and was not read.
>
> Fixed to mirror `api/venue/bookings` exactly rather than invent a third rule: only `walk-in` skips the same-day check, so a visit, a group and a single booking now answer identically. **A staff `phone` booking still gets the guest rule**, matching singles; whether that is right is a live product question and is deliberately not answered by a regression fix.
>
> **Recorded rather than glossed:** `source` is client-supplied and these routes have no staff auth, so claiming `walk-in` bypasses the same-day rule. That trust boundary already existed on this field for compliance and entitlement decisions; this joins it. Closing it means authenticating staff sources, which is its own piece of work and is **not** scheduled.
>
> The bypass guard passed throughout the defect, because it asserted the call was *present* and the defect was that it was *unconditional*. It now pins the source split.
...
| **SA-M2** | **CLOSED 2026-08-16 (`79d2d889` + `cb51f514` + `995e8803`), both limbs.** The **diary lockout** went first: a closure day's bookings are clickable again and staff can create on a closed day, which needed the hit-testing fix as well as the rule (see `SA-H3`). **Reminders** followed: all three lanes now read closure blocks and skip a booking whose start the owner has closed over. **The design decision worth carrying:** the obvious implementation asks `resolveVenueWideAllowedMinuteRanges` whether the venue is open, and it is wrong — that resolver returns `closed` both for a closure the owner made *and* for a weekday with no opening periods, and `SA-H5` establishes that staff deliberately book in the second case. Suppressing those would have quietly undone shipped behaviour. So it reads closure blocks only, never the weekly schedule, and tests the booking's **start** rather than its window (a booking running past a closure boundary is that same override seen from the other end). It **fails towards sending**, deliberately and unlike the other 49 fail-open reads, because a needless reminder is a phone call and a withheld one is a guest who does not arrive; failures still report via the `SA-C3` reporter. Suppression is a `continue` **before** the send, pinned by test, so no `communication_logs` row is written and the dedupe slot stays free — otherwise deleting a closure mid-window would permanently silence that reminder. **What remains is Phase 5, not this finding:** notification, bulk move, bulk cancel and refund. Downgraded from Critical on re-verification: there is **no automated no-show marking anywhere**, so that consequence was speculative. | `src/lib/calendar/closure-booking-conflicts.ts:277` |
...
6. **`SA-H7`'s fix broke staff bookings, and the false premise was written into the fix's own comment.** "Both this route and `create-group` are anonymous public flows" — they are also the staff mobile app's create path. An unconditional guest check refused walk-ins, which are always today, on every venue with `allow_same_day_booking: false`. The correct helper sat uncalled in the same file and both routes already branched on `source` for three other purposes. **The routes were read for what they enforce and never for who calls them.** Fixed in `51d00fd6`; see `SA-H7`.
...
**And enumerate the CALLERS, not just the layers.** Item 6 is the same mistake rotated: every layer was found and the fix was still wrong, because "who invokes this route" was assumed rather than checked. A route with no authentication is not therefore a public-only route. **Three of the six errors here** — `SA-H3`'s hit-testing, `SA-H5`'s server gate, `SA-H7`'s staff caller — are a correct rule applied to an incomplete map of who touches it.
...
| Suppress reminders for bookings inside a closure (`src/lib/cron/unified-scheduling-comms.ts`) | part of `SA-M2` | **S** | ✅ `995e8803`, staging, hand-tested. Applied to all three reminder lanes |
...
> **SUPERSEDED, and in scoping as of 2026-08-16. Do not schedule from the text below.** Reconnaissance falsified five of its premises: it bundles three unrelated programmes, it instructs building a resolver that already exists, it picks the semantic that contradicts the shipped UI promise, it calls a 2-test file "well tested", and it never maps the calendar layer — which is larger than the venue layer and contains two competing break models. The four resolver-using engines also disagree with each other on weekly-closed days, which this document does not record. See **PHASE 1 IS IN SCOPING** in the implementation status at the top for the corrected picture and the operator decisions already taken.
