...
const BUCKET = 'venue-team-photos';
const MAX_SIZE = 5 * 1024 * 1024; // 5MB
const ALLOWED_TYPES = ['image/jpeg', 'image/png', 'image/webp'];

/** POST /api/venue/team-photo - upload a "Meet the team" member photo (admin only). Returns public URL. */
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('POST /api/venue/team-photo failed:', err);
