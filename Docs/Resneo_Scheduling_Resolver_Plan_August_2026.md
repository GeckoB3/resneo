...
**⚠️ What is NOT verified.** The injection used to prove the first route did not run on this one: `fetchAppointmentInput` is not on that request's path, so no failure was ever raised inside the wrap and **neither the 503 nor the card was observed on this route**. The code is written and typechecks; that is all that can be claimed. Before this ships, find a read the appointment branch genuinely performs, inject there, and confirm both the 503 and the card. Do not take the first route's success as evidence for this one.
