...
Guest money is charged on your **Stripe Connect** account (direct charges). ReserveNI never holds the deposit.
...
## ReserveNI subscription vs Stripe Connect

These are separate:

- **Settings → Plan** is your **ReserveNI** subscription (Light, Plus, Pro, Restaurant, Founding Partner). If that is **past due** or fully ended, **dashboard writes** can be blocked and **public online booking** may be paused depending on tier (see the availability troubleshooting article).
...
Until Connect reaches the **active** state shown in app, assume guests cannot complete card steps even if your **ReserveNI** subscription is healthy.
...
**Settings → Plan** covers your **ReserveNI** subscription. **Past due** blocks **dashboard mutations** for all tiers. **Public** booking is paused when the subscription has **fully ended** (and for **Appointments Light** combined with **past due**, which is not your tier, but listed here so you understand mixed documentation if you ever downgrade test venues).
...
Map each source column to the correct ReserveNI field. Double check **timezone** interpretation for datetime columns if your export was generated in UTC.
...
Appointments venues often import **clients** and **bookings** that reference services or calendars. Validation will fail if practitioner or service ids from the old system do not map cleanly to ReserveNI ids. Use the mapper UI carefully.
