 ...
 * joined the list when P1-5 created it. `/account/security` left it when P1-3
 * folded password, sessions and deletion into the profile page, so five routes
 * now carry the whole portal where thirteen did.
 */

const ACCOUNT = path.join(process.cwd(), 'src', 'app', 'account');

/** The routes that survive P1-3 and P1-5, relative to `src/app/account`. */
const SURVIVING = ['', 'bookings', 'bookings/[bookingId]', 'passes', 'profile'];
