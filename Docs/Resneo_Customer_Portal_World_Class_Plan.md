...
**Status at 2026-08-27: F1, F2, F3, F4, F6 and F8 are done; F5 and F9 are partly done; F7 is not started.** F8 turned out to be four defects rather than one, all fixed and deployed: magic-link and password-reset sign-in work on web and in the shipped app. F4's three decisions were taken (see the decisions block above); **two of them are code-complete but still need applying by hand in both Supabase dashboards**, because `config.toml` does not configure hosted projects. **F5 is the gating item**: the smoke suite passes locally but its first CI run failed, and the safety net does not exist until CI is green. F7 still needs credentials this plan does not have.
...
| **F5 ◐** | **PARTLY DONE 2026-08-27. The suite runs and passes locally; CI does not.** The specs had drifted badly from the product and all four failed on their first real run: six separate mismatches, none of them product defects, fixed in `ed1b8433` (see the **2026-08-27 (e2e)** changelog row). They now pass locally, 10 tests in ~50s, against a dev server **and** against a production build. **The first CI run still failed all four paid specs**, on a 60s timeout waiting for the confirmation heading. Not reproduced locally, including with the dotenv files hidden and only the workflow's env passed, which is the closest local approximation of the runner. `5ef8c488` makes a rejected payment fail with Stripe's own message instead of a silent timeout, so the next run's log should name the cause. **Still owed: a green CI run, and `Docs/E2E_SMOKE.md`.** | The safety net does not exist until CI is green. One repaired defect is a warning about the rest: the reschedule spec had the wrong ARIA role behind a `count()` check, so it silently skipped picking a date and asserted a reschedule that never changed the time. A spec that passes while testing nothing is worse than one that fails. | | ◐ |
