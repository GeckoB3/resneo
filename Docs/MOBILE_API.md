...
Since 2026-09-12 `GET /api/venue/dashboard-home` counts a multi-service visit once per day, as the appointments list draws it (R36): `today.bookings`, `today.confirmed`, `today.pending`, `today.seated`, `today_by_booking_model` and `forecast[].bookings` count visits, and `recent_bookings` lists a visit once (its earliest service that day, carrying the visit's status) with a new `group_booking_id` field. `today.covers`, `today.revenue`, `next_booking` and the load figures still read every service. A party's people and a class cart's sessions still count one by one.

Since 2026-09-19 `GET /api/venue/dashboard-home` also returns `new_bookings`, additively: the bookings MADE today, this week (from Monday) and this month on the venue's clock, whatever date each is for, as `{ today, this_week, this_month, week_start, month_start }`. Each count is `{ total, by_channel: { online, team, walk_in, linked_venue }, cancelled, awaiting_payment }`: `total` includes bookings since cancelled (`cancelled` says how many of them), `awaiting_payment` (still Pending on a deposit or card) is not in `total`, a multi-service visit counts once, and imports, collective move copies and bookings the system cancelled for non-payment are left out. It is `null` when the figure could not be loaded, and absent from older servers. The web shows it as the New bookings card on Home. The same figures by day, week or month come from the admin-only `GET /api/venue/reports/new-bookings` with `preset=today|yesterday|this_week|last_week|this_month|last_month` (or `from` and `to`) and `grain=day|week|month`. See `src/lib/reports/new-bookings.ts`.

Since 2026-09-19 `GET /api/venue/reports` keeps its shape, but three reports mean something new, so the app's Reports screen shows the new figures without a release. `report1_booking_summary` and `report3_cancellation` count the bookings MADE from `from` to `to` on the venue's own days exactly as `new_bookings` does, so `total_bookings_created` is `new_bookings`' `total` for the same dates: a multi-service visit counts once, and imports, collective move copies and payment lapses are left out. `covers_booked` counts people (a visit is one person), `covers_seated` the ones who have arrived, started or finished, `by_source` keeps its raw source keys but counts bookings, and `by_status` is one entry per booking from its current status, where `Pending` is the bookings still waiting for payment (not in the total). In `report3_cancellation`, `cancelled_guest_initiated` is the bookings the client cancelled themselves, `cancelled_auto` the system's non-payment cancellations (not in the total or the rate), and `cancellation_rate_pct` is every client or team cancellation over `total_bookings_created`, to 2 dp. Two fields are additive: `report1_booking_summary.by_channel` (`{ online, team, walk_in, linked_venue }`, the New bookings split, which the web pie now uses) and `report3_cancellation.cancelled_team_initiated` (the web shows it as a Team-initiated tile; the app's Cancellation card has no row for it yet). `report2_no_show_series` is now by appointment date, one row per day that had an outcome, from current statuses, with a visit counted once a day; `report8_baseline_metrics.no_show` counts the same way. Before, these came from SQL functions that counted every service row on UTC days and could not see a status the team changed. See `src/lib/reports/overview-bookings.ts` and `src/lib/reports/no-show-series.ts`.
...
## All seven per-calendar values are stored (2026-09-15)

`PATCH /api/venue/practitioner-service-overrides` now stores all seven values on a unified-scheduling venue: `custom_name`, `custom_description`, `custom_duration_minutes`, `custom_buffer_minutes`, `custom_price_pence`, `custom_deposit_pence` and `custom_colour`. Before this only length and price were kept; the other five were dropped when sent with them and refused with a 400 otherwise. Staff still need the service's matching `staff_may_customize_*` flag (403 per field without it). A venue admin may now call the route too, for any calendar at their venue whatever the flags say, and must send `calendar_id` (400 without it, 403 for a calendar at another venue). On a card-hold service a deposit below 100 pence is refused with a 400; `null` clears a value. A name of only spaces is a 400.

`GET /api/venue/appointment-services` returns each calendar's values in `practitioner_services` instead of five hard-coded nulls. Each of the seven is returned only while the service's matching `staff_may_customize_*` flag is on, and null otherwise. From 2026-09-15 that includes length and price (D56): a calendar's stored price or length no longer applies once its flag is turned off, and bookings are priced at the service's own. The app's existing `link ?? base` merge is correct unchanged, and will now show a calendar's own name, buffer and deposit where one is set. Requires migration 20270214140000 on the environment.

## Phone and walk-in visits and groups need a staff session (2026-09-14)

`POST /api/booking/create-multi-service` and `POST /api/booking/create-group` now refuse a `phone` or `walk-in` source without a staff session that may book at the venue: 401 `Phone and walk-in bookings are for signed-in staff only.` with no session, 403 `You cannot take bookings for that venue.` for staff of an unrelated venue. Staff of the venue, members of the live collective the booking goes through, and partners holding a `create_edit_cancel` link are accepted, as for override availability. Public sources are unchanged. The app already sends its Bearer token on these calls, so nothing changes for a signed-in app; the rows now carry `created_by_staff_id` (and `created_by_linked_venue_id` when the staff member belongs to another venue), like single staff bookings.
