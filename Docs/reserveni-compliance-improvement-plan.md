...
Steps 1 and 2 are now implemented (see the implemented notes above): they covered
the biggest staff and guest pain points with no migration. Step 3 (draft
persistence) is the recommended next pickup. Steps 4 and 5 are the only remaining
ones that touch the data model or shared message copy; everything else is
front-end-only.
