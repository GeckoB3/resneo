...
here and about 31,000 lines. This page exists so you can tell which one owns your
...
| `collective-one-venue-plan.md` | Not implemented. Written 2026-09-13 from a multi-agent forensic audit of venue collectives (eight subsystem maps, a cross-examination, two competing architectures, two red teams). Records how collectives work today, 42 split-brain cases and 62 bugs, and recommends host-managed replicas: the host's service is the master, members hold locked copies written by one audited engine, calendar assignments are the only record of which calendar offers what, and leaving is lossless. Companions: `collective-one-venue-ux-spec.md` (pages and copy) and `collective-one-venue-test-plan.md` (147 tests, invariants, rollout gates). Decisions D1 to D51 are the owner's; D9 asks for legal counsel before build. Supersedes `collective-service-sync-plan.md` in design. Second forensic pass 2026-09-14 at `c0b5eb0` widened it to reporting, operations, multi-venue people and plan tiers |
| `collective-one-venue-ux-spec.md` | Not implemented. Companion to `collective-one-venue-plan.md`: the page-by-page specification. Says exactly what a host, a member and a guest see and can do on every surface, with every string of copy, plus the "where is each fact edited" matrix, the lifecycle journeys J1 to J12, the notification inventory N1 to N31, accessibility and mobile, and the open questions still waiting on an owner decision. Read it with the plan, never alone: it names decisions by id without restating them |
| `collective-one-venue-test-plan.md` | Not implemented. Companion to `collective-one-venue-plan.md`: the testing plan. What test infrastructure exists today and where it is blind, the strategy and fixtures, the full inventory of tests with a location each, the SQL invariants (I1 to I33) run identically by pgTAP, the daily verifier, the migration script and every deploy step, the rollout gates per pass, the owner's acceptance checklist written in plain language against R1 to R14, and the CI changes |
