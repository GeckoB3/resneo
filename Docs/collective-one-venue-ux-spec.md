...
Footer `svc.member.view.close`, `svc.member.view.save` (enabled when changed). Retired copies add `svc.member.view.retiredNote`. PATCH sends only `practitioner_ids`, `online_meeting_url`, `online_meeting_info` and `capacity_per_session`.
...
New component `src/components/linked-accounts/collective/CreateCollectiveDialog.tsx`, replacing `CreateCollectiveModal` (`VenueCollectivesPanel.tsx:382-544`). It follows the house wizard shape already used by `RestaurantSetupWizard.tsx` (numbered steps, a progress rail, a validation gate before advancing, a preview step, then a done step), built from the primitives rather than that file's legacy Tailwind.
