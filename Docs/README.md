...
| `collective-service-sync-plan.md` | SUPERSEDED in design by `collective-one-venue-plan.md`, but still the as-built record of what runs today. Implemented on staging 2026-09-09 (its Progress section is that record): members' service copies made by the combined-page tick follow the origin's scheduling shape (duration, buffer, processing periods, variants); a member's edit detaches the copy visibly; the manager shows in step / customised / independent; not retroactive. Migration `20270209120000_service_sync_from_origin.sql` owed to both environments; the code tolerates its absence. Read it to understand today's behaviour, not to build from: the replicas design replaces sync entirely. A collective stops using sync when it is migrated (`scripts/collective-replicas-migrate.mjs`, W9), and the sync code goes in W14 once every collective has moved |
