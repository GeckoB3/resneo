...
export async function POST(request: NextRequest, ctx: RouteCtx) {
  try {
    ...
    const { data: updated, error } = await staff.db
      ...
      .single();
    if (error) {
      console.error('archive compliance type failed:', error.message);
      return NextResponse.json({ error: 'Failed to archive type.' }, { status: 500 });
    }
    ...
    console.error('POST /api/venue/compliance/types/[id]/archive failed:', err);
