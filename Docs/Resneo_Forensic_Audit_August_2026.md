...
That last row matters: the RLS security suite has never been continuously verified, and the "green suite" figure covers none of it. **It is still true after C0-C13 and N2/N3/N4**, which is the single largest residual risk in this document: four RLS-touching changes have now shipped to production verified by reasoning, targeted probes and unit tests, never by the suite written for exactly that purpose. Wiring it up is step 3 of the remaining work.
...
>
> **N2's audit gap** is closed by extending `cross_venue_booking_audit_trigger` to `DELETE`. The trap the document flags is real and was handled: on `DELETE`, `NEW` is NULL, so the original `NEW.venue_id` guard raises before any branch runs; every reference now routes through locals resolved from the right record. **Two further traps it does not mention, either of which would have made this a change that succeeds and does nothing:** the notification mapper is called `notify_owning_venue_of_cross_venue_write`, not the name a first pass guessed, and — more importantly — its trigger carries a `WHEN` clause listing the three action types it fires for, so `deleted_booking` would never have reached the mapper at all. Both are recreated. A deletion notifies as `cross_venue_booking_cancelled` rather than a new type the notification UI cannot render.
>
> **N4** is closed with `link_guest_calendar_allows(owner_venue, guest)`, a `SECURITY DEFINER` helper matching every other `link_*` function: a guest is visible when they hold a booking on a calendar the link covers. It reuses `COALESCE(calendar_id, practitioner_id)`, the exact expression the four bookings policies already use, so it introduces no new assumption about what a granted id list contains. **This is the thirteenth client-executable function**, and the allowlist in `scripts/check-client-executable-functions.mjs` was updated in the same commit; `npm run check:function-grants` returns **PASS at 13** against staging.
>
> **`practitioners` and `appointment_services` are deliberately left unscoped.** Both are named in N4's body, but neither carries personal data, and scoping `practitioners` turns on a fact not yet established: whether a granted calendar-id list holds practitioner ids or unified-calendar ids decides whether the scope hides one row or every row. Nothing in `src/` reads either table through a browser client, so the residue is direct-PostgREST only. Recorded rather than guessed at.
>
> **Verified by behaviour.** The headline regression risk was the `OLD`-aware rewrite, since getting it wrong makes *every* booking delete throw. Probed on staging: a throwaway row was created and deleted successfully, the row is gone, `link_guest_calendar_allows` returns true for a guest with bookings and false for one without. `tsc` clean, lint 0 errors, **331 files / 3132 tests** green.
>
> **pgTAP updated in the same commit, as A5 requires.** Three assertions encoded the vulnerability as expected behaviour: tests 4 and 5 asserted a direct cross-venue UPDATE succeeds and audits, and test 21 asserted a `create_edit_cancel` partner can INSERT directly. All three now assert refusal, with a companion check that the target row is unchanged. The suite still runs nowhere, so this is correctness of the record, not a passing gate.
...
> **A5 — WIRED, 2026-08-14. The suite had never run, and the reason was not the one this document assumed.**
>
> A `rls-pgtap` job now starts a **local** Supabase built from `supabase/migrations` and runs `supabase test db`. No production credential, so this public repository needs no Actions secrets, and it catches a bad migration before it reaches any environment — which a check pointed at a live project cannot do. `npm run test:rls`, `db:start` and `db:stop` mirror it locally.
>
> **The suite was broken at fixture setup, not at three assertions.** A5 says "exactly three assertions fail". That analysis assumed the file otherwise ran. It could not: the guests fixture inserted `guests.name`, a column **dropped by `20260810120000` on 2026-08-10, three days before this audit was written**. Postgres errors there, before `plan(22)` reaches a single test. So the suite has been dead since well before any of this work, and nobody could have noticed, because nothing executed it. Every fixture column was then checked against the live schema rather than eyeballed; `guests.name` was the only casualty. The three assertion changes A5 predicted were made earlier, with N2/N3/N4.
>
> **Honest limit: this has not been executed.** Docker is unavailable on the machine this was written on, so the job could not be run locally. The first CI run is the verification, and it is expected to surface further drift — the file is a year of schema change away from the last time it was known to work. It is deliberately a **blocking** job rather than `continue-on-error`: a green-when-broken check is worse than no check, and that is the whole lesson of C0.
>
> **`check:function-grants` is deliberately NOT hosted here yet**, though this document proposes doing both at once. C0's root cause was that **hosted** Supabase grants `anon`/`authenticated` EXECUTE through project-level default privileges. Whether a local instance reproduces those defaults is unestablished, and if it does not, the allowlist of 13 simply will not hold locally and the check would fail for a reason that has nothing to do with drift. Assuming the two environments agree is exactly what produced C0. Verify against a running local instance first; the infrastructure is now there to do it.
