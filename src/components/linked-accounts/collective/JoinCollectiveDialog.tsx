...
type Step = 'means' | 'services' | 'forms' | 'check';
...
export function JoinCollectiveDialog({ open, onClose, collectiveId, venueName, onJoined }: JoinCollectiveDialogProps) {
  ...
  const [sameName, setSameName] = useState<Record<string, 'use_mine' | 'add_new'>>({});
  const [optionMap, setOptionMap] = useState<Record<string, string | null>>({});
  const [own, setOwn] = useState<Record<string, 'ask' | 'park'>>({});
  const [forms, setForms] = useState<Record<string, 'use_existing' | 'use_theirs'>>({});
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        setPreview(data);
        // Defaults: use the venue's own same-name service, match options by name, park the rest,
        // and keep the venue's existing forms. Each is one click to change.
        setSameName(Object.fromEntries(data.same_name.map((s) => [s.item_id, 'use_mine'])));
        const map: Record<string, string | null> = {};
        for (const pair of data.same_name) {
          for (const mine of pair.my_options) {
            map[mine.id] = pair.host_options.find((h) => h.name.trim().toLowerCase() === mine.name.trim().toLowerCase())?.id ?? null;
          }
        }
        setOptionMap(map);
        setOwn(Object.fromEntries(data.own_services.map((s) => [s.id, 'park'])));
        setForms(Object.fromEntries(data.forms.map((f) => [f.host_type_id, 'use_existing'])));
      } catch {
  ...
  const steps: Step[] = useMemo(
    () => (preview && preview.forms.length > 0 ? ['means', 'services', 'forms', 'check'] : ['means', 'services', 'check']),
    [preview],
  );
  ...
  const choices = (): JoinChoices => ({
    same_name_choices: (preview?.same_name ?? []).map((pair) =>
      sameName[pair.item_id] === 'use_mine'
        ? {
            item_id: pair.item_id,
            choice: 'use_mine',
            my_service_id: pair.my_service_id,
            option_map: pair.my_options.map((o) => ({ my_variant_id: o.id, host_variant_id: optionMap[o.id] ?? null })),
          }
        : { item_id: pair.item_id, choice: 'add_new' },
    ),
    own_service_choices: Object.entries(own).map(([service_id, choice]) => ({ service_id, choice })),
    form_choices: (preview?.forms ?? []).map((f) =>
      forms[f.host_type_id] === 'use_existing'
  ...
  const join = async () => {
    ...
    try {
      const res = await fetch(`/api/venue/collectives/${collectiveId}/members`, {
        ...
        body: JSON.stringify({ action: 'accept', consent_version: preview.consent_version, ...choices() }),
      });
      if (!res.ok) {
  ...
  const counts = {
    useMine: Object.values(sameName).filter((c) => c === 'use_mine').length,
    park: Object.values(own).filter((c) => c === 'park').length,
    ask: Object.values(own).filter((c) => c === 'ask').length,
  };
  ...
  return (
    <Dialog
      ...
      description={
        preview ? collectiveCopy('join.step', { n: index + 1, total: steps.length }) : undefined
      }
      size="lg"
      footer={
                    ...
                    <fieldset key={pair.item_id} className="rounded-lg border border-slate-200 p-3">
                      ...
                      <label className="flex items-center gap-2">
                        <input
                          ...
                          checked={sameName[pair.item_id] === 'use_mine'}
                          onChange={() => setSameName((prev) => ({ ...prev, [pair.item_id]: 'use_mine' }))}
                        />
                      ...
                      <label className="flex items-center gap-2">
                        <input
                          type="radio"
                          name={`same-${pair.item_id}`}
                          checked={sameName[pair.item_id] === 'add_new'}
                          onChange={() => setSameName((prev) => ({ ...prev, [pair.item_id]: 'add_new' }))}
                        />
                      ...
                      {sameName[pair.item_id] === 'use_mine' ? (
                        <div className="mt-2 space-y-2">
                          <p className="text-xs text-slate-600">
                            {collectiveCopy('join.services.useMine.note', { service: pair.name, host })}
                          </p>
                          {pair.my_options.length > 0 ? (
                            <table className="w-full text-xs">
                              <caption className="text-left font-medium text-slate-800">
                                {collectiveCopy('join.map.heading')}
                              </caption>
                              <thead>
                                  ...
                                  <tr key={mine.id}>
                                    ...
                                    <td className="py-1">
                                      <select
                                        ...
                                        value={optionMap[mine.id] ?? ''}
                                        onChange={(e) =>
                                          setOptionMap((prev) => ({ ...prev, [mine.id]: e.target.value || null }))
                                        }
                  ...
                  preview.own_services.map((service) => (
                    <div key={service.id} className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-100 py-1">
                      ...
                      <select
                        ...
                        value={own[service.id] ?? 'park'}
                        onChange={(e) =>
                          setOwn((prev) => ({ ...prev, [service.id]: e.target.value as 'ask' | 'park' }))
                        }
                ...
                <fieldset key={form.host_type_id} className="rounded-lg border border-slate-200 p-3">
                  ...
                  <label className="flex items-center gap-2">
                    <input
                      ...
                      checked={forms[form.host_type_id] === 'use_existing'}
                      onChange={() => setForms((prev) => ({ ...prev, [form.host_type_id]: 'use_existing' }))}
                    />
                  ...
                  <label className="flex items-center gap-2">
                    <input
                      type="radio"
                      name={`form-${form.host_type_id}`}
                      checked={forms[form.host_type_id] === 'use_theirs'}
                      onChange={() => setForms((prev) => ({ ...prev, [form.host_type_id]: 'use_theirs' }))}
                    />
          ...
          {step === 'check' ? (
            <section className="space-y-2">
              ...
              <ul className="list-disc space-y-1 pl-5">
                ...
                {counts.useMine > 0 ? (
                  <li>{collectiveCopy('join.summary.useMine', { count: counts.useMine, host })}</li>
                ) : null}
                {counts.park > 0 ? (
                  <li>{collectiveCopy('join.summary.park', { count: counts.park, collective })}</li>
                ) : null}
                {counts.ask > 0 ? <li>{collectiveCopy('join.summary.ask', { count: counts.ask, host })}</li> : null}
              </ul>
