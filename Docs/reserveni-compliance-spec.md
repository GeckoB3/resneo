...
**Performance:** the "Missing for upcoming bookings" query joins `bookings`, `service_compliance_requirements`, and `compliance_records` for a 14-day window and can be expensive at scale. The dashboard route (`GET /api/venue/compliance/dashboard`) should compute this in a single CTE, leveraging the `idx_bookings_venue_date` composite index on `bookings` and the partial index `(venue_id, expires_at) WHERE status = 'completed'` on `compliance_records`. For venues with high booking volume, cache the dashboard response server-side for up to 5 minutes, keyed on `venue_id`. **As built (2026-09-13): no cache.** The 5-minute in-memory cache kept a newly made booking off the sweep, and the routes that change bookings and records could not clear it, so the route is `no-store`; the loader reads the venue's requirements first and then only the bookings they apply to. The same pass fixed the page having never shown anything: all three reads embedded `guests.name`, which the first/last name migration had dropped, and the errors were swallowed into "You're all caught up". Rows are one per client, day and form, and today's check-ins are one card per client.
