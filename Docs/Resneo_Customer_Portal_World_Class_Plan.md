...
**Status at 2026-08-27: F1, F3, F4, F6 and F8 are done; F2 and F9 are mostly done. F8 turned out to be four defects rather than one, all fixed and deployed: magic-link and password-reset sign-in work on web and in the shipped app. F4's three decisions were taken on 2026-08-27 (see the decisions block above). **F5 is now the gating item**, and it needs only the seven credential secrets in F2. F7 still needs credentials this plan does not have.**
...
| **F2 ◐** | **MOSTLY DONE 2026-08-27.** `.env.e2e.example` committed with a `.gitignore` negation. Both fixture venues now seeded on **staging** (`e2e-smoke-appointments`, `e2e-smoke-staff-first`), each carrying Connect account `acct_1Tb120QhIGmL7r1A`, verified on the venue rows. Local `.env.e2e` written (gitignored). Repo variable `E2E_STAFF_FIRST_VENUE_SLUG` and the five non-credential secrets (`E2E_VENUE_SLUG`, `E2E_VENUE_NAME`, `E2E_SERVICE_NAME`, `E2E_OPTIONS_SERVICE_NAME`, `E2E_STRIPE_CONNECTED_ACCOUNT_ID`) are set. **`RUN_E2E_SMOKE` is deliberately `false`**: turning it on before the credential secrets exist would fail every CI run. **Still needed from the operator: the seven credential secrets** (`NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`, `SUPABASE_SECRET_KEY`, `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, `NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY`, `PAYMENT_TOKEN_SECRET`), all pointing at **staging**, then flip `RUN_E2E_SMOKE` to `true`. | Without `E2E_VENUE_SLUG` every appointment spec **skips**, so the job would pass having tested nothing (`global-setup.ts:12-17`). The app reads `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY ?? ..._ANON_KEY`, so the workflow's `ANON_KEY` name works and takes the publishable value. | Operator | ◐ |
