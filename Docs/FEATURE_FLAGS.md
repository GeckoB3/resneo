...
Admins on appointment-capable venues: **Settings → Profile → Optional Booking features** toggles per-venue overrides.

Not every flag has a card there. `staff_first_booking_flow` is deliberately absent for now: it is set per venue from the super admin flags page (or the env override) until its settings card ships in a later release.
