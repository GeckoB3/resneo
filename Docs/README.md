...
| `collective-staff-booking-plan.md` | Implemented on staging 2026-09-04 (spec §8.7): the staff booking form books for a live venue collective from every entry point by reusing the single-partner `linkedOwnerVenueId` path with the four staff routes made collective-aware; the plan records the investigation and the decisions taken. Reviewed 2026-09-05: members' own services added to the staff catalogue, linked column button removed for collective calendars, bridge booking window fixed; second pass the same day: suspended links no longer remove members, partner reschedules validated, catalogue memoised (spec §8.7) |
