...
- Card anatomy: 48px avatar (`h-12 w-12`; photo when shown, else initial), name, owning venue subtitle on combined pages (4.7), up to 3 specialties chips plus a "+N" overflow chip, and a single-line clamped bio when present (guests choose a person here; a one-line bio reduces the temptation to tab away, see the tab-switch note below). No prices (no service context yet).
...
- **Venue attribution (new in revision 2)**: `loadCollectiveAppointmentCatalog` adds `owning_venue_name` to each practitioner (additive field; the venue-name lookup already exists for duplicate-name qualification, 361-377, and now always runs). `staff_pick` cards on combined pages show it as the subtitle for every card, not only for duplicates: which venue a person works at is decision-critical before any service context exists. The duplicate-name suffix on `name` itself is retained for downstream surfaces (banners, summaries).
