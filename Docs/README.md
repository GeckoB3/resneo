...
| `collective-service-sync-plan.md` | Implemented on staging 2026-09-09 (its Progress section is the as-built record): members' service copies made by the combined-page tick follow the origin's scheduling shape (duration, buffer, processing periods, variants); a member's edit detaches the copy visibly; the manager shows in step / customised / independent; not retroactive. Migration `20270209120000_service_sync_from_origin.sql` owed to both environments; the code tolerates its absence |
...
| `collective-one-venue-plan.md` | Not implemented. Written 2026-09-13 from a multi-agent forensic audit of venue collectives (eight subsystem maps, a cross-examination, two competing architectures, two red teams). Records how collectives work today, 27 split-brain cases and 55 bugs, and recommends host-managed replicas: the host's service is the master, members hold locked copies written by one audited engine, calendar assignments are the only record of which calendar offers what, and leaving is lossless. Companions: `collective-one-venue-ux-spec.md` (pages and copy) and `collective-one-venue-test-plan.md` (113 tests, invariants, rollout gates). Decisions D1 to D36 are the owner's; D9 asks for legal counsel before build. Would supersede `collective-service-sync-plan.md` |
