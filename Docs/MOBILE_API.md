...
`GET /api/venue/appointment-services` returns each calendar's values in `practitioner_services` instead of five hard-coded nulls. Name, description, buffer, deposit and colour are returned only while the service's flag is on (null otherwise); length and price as stored, as before. The app's existing `link ?? base` merge is correct unchanged, and will now show a calendar's own name, buffer and deposit where one is set. Requires migration 20270214140000 on the environment.
