...
Steps 1 to 4 are now implemented (see the implemented notes above): they covered
the biggest staff and guest pain points with no migration. **Step 5 (staff
management niceties: version history + changelog + restore, Duplicate,
permanent-result-type warning, single-request save) is the recommended next
pickup** and is the only remaining step that touches the data model or schemas;
everything in Step 6 is front-end-only polish.
